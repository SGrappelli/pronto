...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const canonical = `https://trypronto.app/es/blog/${post.slug}`

  const languages: Record<string, string> = { es: canonical }
...
export default function EsBlogPostPage({ params }: { params: { slug: string } }) {
  const post = getPost('es', params.slug)
  if (!post) notFound()
...
<nav>
  ...
  <div class="nav-right">
    ...
    <a href="/register" class="btn-nav" onclick="window.gtag&&window.gtag('event','sign_up_click',{label:'start_free',page:'/es/blog/${post.slug}'})">Empezar gratis</a>
  </div>
</nav>

<div class="breadcrumb">
  <a href="/es/">Inicio</a><span>&rsaquo;</span>
  <a href="/es/blog">Blog</a><span>&rsaquo;</span>
  ${post.title}
</div>
...
<main>
  <div class="article-wrap">
    ...
    <div class="article-cta">
      <h2>Cero comisiones, para siempre</h2>
      <p>Plan gratuito para siempre. Sin tarjeta de cr&eacute;dito. Sin comisi&oacute;n en ning&uacute;n plan.</p>
      <a href="/register" class="btn-cta-white" onclick="window.gtag&&window.gtag('event','sign_up_click',{label:'article_cta',page:'/es/blog/${post.slug}'})">Probar Pronto gratis &rarr;</a>
    </div>
  </div>
</main>
