...
export function generateStaticParams() {
  return getPosts('pt').map(post => ({ slug: post.slug }))
}
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const canonical = `https://trypronto.app/pt/blog/${post.slug}`

  const languages: Record<string, string> = { 'pt-BR': canonical }
  if (post.translations?.en) languages.en = `https://trypronto.app/blog/${post.translations.en}`
...
export default function PtBlogPostPage({ params }: { params: { slug: string } }) {
  const post = getPost('pt', params.slug)
  if (!post) notFound()
  ...
  const langSwitcherMap = JSON.stringify({
    ...
    es: post.translations?.es ? `/es/blog/${post.translations.es}` : '/es/blog',
...
<nav>
  ...
  <div class="nav-right">
    ...
    <a href="/register" class="btn-nav" onclick="window.gtag&&window.gtag('event','sign_up_click',{label:'start_free',page:'/pt/blog/${post.slug}'})">Comece gr&aacute;tis</a>
  </div>
</nav>

<div class="breadcrumb">
  <a href="/pt/">In&iacute;cio</a><span>&rsaquo;</span>
  <a href="/pt/blog">Blog</a><span>&rsaquo;</span>
  ${post.title}
</div>
...
<main>
  <div class="article-wrap">
    ...
    <div class="article-cta">
      <h2>Zero comiss&atilde;o, para sempre</h2>
      <p>Plano gratuito para sempre. Sem cart&atilde;o de cr&eacute;dito. Sem comiss&atilde;o em nenhum plano.</p>
      <a href="/register" class="btn-cta-white" onclick="window.gtag&&window.gtag('event','sign_up_click',{label:'article_cta',page:'/pt/blog/${post.slug}'})">Experimentar Pronto gr&aacute;tis &rarr;</a>
    </div>
  </div>
</main>
